...
export default function IFour({ relatedSubCats, category }: MegaMenuProps) {
  ...
  return (
    <MegaMenuWrapper className="border-t lg:translate-x-[-15%] xl:translate-x-[-66%] lg:w-[1500px] xl:w-[2200px] 3xl_v2:w-[2400px] 4xl:w-[3500px] 10xl:w-[6000px] lg:h-[255px] xl:h-[320px] flex justify-center py-[40px]">
      <div className="w-full md:max-w-[90vw] 3xl:max-w-[86.56vw] 7xl:max-w-[62vw] 10xl:max-w-[50vw] mx-auto h-full flex items-stretch xl:pl-44">
        ...
        {/* Content */}
        <div className="ml-6.5 flex-1 h-full overflow-hidden">
              ...
              <div key={columnIndex} className="space-y-1.5">
                {column.map((subCat) => (
                  ...
                  >
                    ...
                    <div className="relative overflow-hidden h-6">
                      <span className="block transition-all duration-200 tracking-[0.5px] [transform:translateY(0)_rotateX(0)] group-hover:opacity-0 group-hover:[transform:translateY(50%)_rotateX(90deg)] text-[#0a0a0ab3]">
                        {subCat.name}
                      </span>

                      <span className="absolute left-0 top-0 block tracking-[0.5px] opacity-0 transition-all duration-200 [transform:translateY(-50%)_rotateX(90deg)] group-hover:opacity-100 group-hover:[transform:translateY(0)_rotateX(0)] text-[#0a0a0ab3]">
                        {subCat.name}
                      </span>
                    </div>
                  </Link>
                ...
                {columnIndex === columns.length - 1 && (
                  <button className="flex items-start gap-4 font-normal text-[16px] 3xl:text-[18px] tracking-[0.5px] leading-6.25 text-[#F05A28] hover:underline underline-offset-4 cursor-pointer">
                    <ArrowRight
                      size={18}
